...
- [x] デスクトップ側で選択ファイルの絶対パスを取得する
- [x] 最近使ったファイルをボタン下部へ一覧表示する
- [x] 履歴からファイルを再度開けるようにする
- [x] 履歴の削除操作を追加する
...
- [x] `~/.reviewers/settings.json` にカラースキーマを保存する
- [x] 起動時に保存済みカラースキーマを復元する
- [x] 最近使ったファイルのパスと利用日時を `summary` に保存する
- [x] 履歴の追加・更新・削除時に設定を永続化する
...
- 現在のローカル設定保存先は `~/.reviewers/settings.json`
